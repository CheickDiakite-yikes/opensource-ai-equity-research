...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Fetching latest SEC filings from ${from || 'N/A'} to ${to || 'N/A'}, limit: ${limit}`);
    
    // Build the API URL based on provided parameters
    let apiUrl = "https://financialmodelingprep.com/stable/sec-filings-financials";
    ...
    const formattedData = filings.map(filing => ({
      ...
      type: `${filing.form} (${filing.filingType || 'SEC Filing'})`,
    ...
    console.error("Error processing latest SEC filings request:", error);
